namespace Hatra.Elastic
{
    ...
    using Hatra.ViewModels.ElasticSearchModels;
    using Nest;
    ...
    public class DataIndexer
    {
        ...
        public async Task<IndexResult> IndexPagesFromFile(PageViewModel[] files, bool deleteIndexIfExists, string index = null)
        {
            SanitizeIndexName(ref index);
            //PageViewModel[] mappedCollection = await ParsePageJsonFile(fileName);
            //await DeleteIndexIfExists(index, deleteIndexIfExists);
            await CreatePageIndexIfItDoesntExist(index);
            await ConfigurePagination(index);
            return await IndexPageDocuments(files, index);
        }

        public async Task<IIndexResponse> IndexMyPages(PageViewModel viewModel)
        {
            var indexResponseAsync = await client.IndexDocumentAsync(viewModel);
        ...
        private async Task<IndexResult> IndexPageDocuments(PageViewModel[] mappedCollection, string index)
        {
            int batchSize = 10000; // magic
            int totalBatches = (int)Math.Ceiling((double)mappedCollection.Length / batchSize);
        ...
        private async Task<PageViewModel[]> ParsePageJsonFile(string fileName)
        {
            using (FileStream fs = new FileStream(Path.Combine(contentRootPath, fileName), FileMode.Open))
            {
                using (StreamReader reader = new StreamReader(fs))
                {
                    ...
                    PageViewModel[] mappedCollection = JsonConvert.DeserializeObject<PageViewModel[]>(rawJsonCollection, new JsonSerializerSettings
                    {
                        Error = HandleDeserializationError
                    });
        ...
        private async Task CreatePageIndexIfItDoesntExist(string index)
        {
            if (!this.client.IndexExists(index).Exists)
            {
                var indexDescriptor = new CreateIndexDescriptor(index)
                                .Mappings(mappings => mappings
                                    .Map<PageViewModel>(m => m.AutoMap()));
